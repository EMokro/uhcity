...
public:
	...
	virtual void Reset()
	{
		...
		#define MACRO_CONFIG_LONGLONG(Name,ScriptName,def,min,max,flags,desc) g_Config.m_##Name = def;
	...
	virtual void RestoreStrings()
	{
		...
		#define MACRO_CONFIG_LONGLONG(Name,ScriptName,def,min,max,flags,desc)	// nop
	...
	virtual void Save()
	{
		...
		#define MACRO_CONFIG_LONGLONG(Name,ScriptName,def,min,max,flags,desc) if((flags)&CFGFLAG_SAVE){ str_format(aLineBuf, sizeof(aLineBuf), "%s %lld", #ScriptName, g_Config.m_##Name); WriteLine(aLineBuf); }
